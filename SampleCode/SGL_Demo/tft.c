/**************************************************************************//**
 * @file     tft.c
 * @brief    ILI9341 TFT 驱动（SGL_Demo / CM2053）— 初始化与 SGL 帧缓冲刷屏回调
 * @details  SPI0：PB12 MOSI、PB13 MISO、PB14 CLK；LCD /CS=PB15(GPIO)；DC=PA6，RST=PA7，背光 EN=PC2。
 *
...
#define SPI_PORT             SPI0  /* PB12 MOSI, PB13 MISO, PB14 CLK */

/* ---------------- SPI0 PDMA (pixel push) ----------------
 * 用 PDMA 把 RGB565 像素流推到 SPI0->TX，可显著减少“可见刷屏”时间。
 * 命令/坐标仍用 8-bit CPU 写；像素阶段切到 16-bit + PDMA_WIDTH_16。
 */
#define TFT_SPI0_PDMA_CH_TX   0

static void tft_spi0_pdma_init_once(void)
{
    ...
    /* 不要 SYS_ResetModule(PDMA0_RST)：SPI0 读 Flash 也占用 PDMA0 其它通道，全模块 reset 会打断/清掉对方 */
    PDMA_Open(PDMA, 1u << TFT_SPI0_PDMA_CH_TX);
}

static int tft_spi0_write_pixels_pdma16(const sgl_color_t *src, uint32_t len)
{
    /* len 是像素个数（16-bit RGB565）。 */
    tft_spi0_pdma_init_once();
    ...
    PDMA_SetTransferCnt(PDMA, TFT_SPI0_PDMA_CH_TX, PDMA_WIDTH_16, len);
    PDMA_SetTransferAddr(PDMA, TFT_SPI0_PDMA_CH_TX,
                         (uint32_t)src, PDMA_SAR_INC,
                         (uint32_t)&SPI_PORT->TX, PDMA_DAR_FIX);
    PDMA_SetTransferMode(PDMA, TFT_SPI0_PDMA_CH_TX, PDMA_SPI0_TX, FALSE, 0);
    PDMA_SetBurstType(PDMA, TFT_SPI0_PDMA_CH_TX, PDMA_REQ_SINGLE, 0);
    PDMA->DSCT[TFT_SPI0_PDMA_CH_TX].CTL |= PDMA_DSCT_CTL_TBINTDIS_Msk;

    /* 不用中断，轮询 TD 标志 */
    PDMA_CLR_TD_FLAG(PDMA, 1u << TFT_SPI0_PDMA_CH_TX);

    SPI_TRIGGER_TX_PDMA(SPI_PORT);

    uint32_t timeout = SystemCoreClock; /* ~1s */
    while (((PDMA_GET_TD_STS(PDMA) & (1u << TFT_SPI0_PDMA_CH_TX)) == 0u) && timeout) {
        timeout--;
    }
...
/*-------- ILI9341：片选 PB15=GPIO（PB12 为 SPI0_MOSI）；DC=PA6；RST=PA7；EN=PC2 ----------------*/
static void ILI9341_CS_Low(void)   { PB15 = 0; }   /* 片选有效 */
static void ILI9341_CS_High(void)  { PB15 = 1; }   /* 片选释放 */
static void ILI9341_DC_Command(void){ PA6 = 0; }  /* RS/DC=0 命令 */
static void ILI9341_DC_Data(void)   { PA6 = 1; }  /* RS/DC=1 数据 */
static void ILI9341_RST_Low(void)  { PA7 = 0; }   /* 复位拉低 */
static void ILI9341_RST_High(void) { PA7 = 1; }   /* 复位释放 */
...
/**
 * @brief 一次 CS 事务内完成：CASET + PASET + RAMWR + RGB565 像素（PDMA 推 SPI0 TX）
                                   ...
                                   const sgl_color_t *src, uint32_t len)
{
    ...
    while (SPI_IS_BUSY(SPI_PORT));
    ILI9341_DC_Data();
    if (tft_spi0_write_pixels_pdma16(src, len) != 0) {
...
static void ILI9341_Init(void)
{
    ...
    PC2 = 1;   /* DISP_SET_BACKLIGHT (active-high) */
}
